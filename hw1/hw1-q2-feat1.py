...
def read_train(train_set, classes, vocabulary):
    """ Generates vocabularies and speakers  """
...
def test_file(lambda_k_w, in_file):
    ...
    for line in in_file:
        ...
        speaker = line[0]
        wc = np.zeros(len(vocabulary) + len(bg_vocab)+ 1)
        for word in line[1:]:
            if word in vocabulary:
                wc[word_lookup[word]] += 1.
        for b in bigramed_line:
            if b in bg_vocab:
                wc[bg_lookup[b]] += 1
        wc[word_lookup['<b>']] = 1
...
def train_model(iterations,learning_rate,lambda_w_k):
    for i in range(0,iterations):
        ...
        for line in train_set:
            ...
            word_counts = np.zeros(len(vocabulary)+len(bg_vocab)+1)
            line = line.split()
            #remove stop-words and symbols
            line = [word for word in line if word not in common_words]
            line = [word for word in line if word not in symbols]
...
if __name__ == "__main__":
    ...
    lambda_w_k = np.zeros((len(classes),len(vocabulary)+len(bg_vocab)+1),dtype=np.float64)
